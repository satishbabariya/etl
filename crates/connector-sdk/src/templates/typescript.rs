...
pub const SOURCE_TEMPLATE_TS: &str = r#"===FILE: package.json===
{
  ...
  "type": "module",
  "scripts": {
    "bundle": "esbuild src/connector.ts --bundle --format=esm --platform=node --outfile=dist/connector.js --external:platform:*",
...
//
// Return raw values from these functions; jco wraps them in result.ok.
// Throw to signal result.err. Do NOT return `{ tag: 'ok', val: ... }`
// yourself — jco does that and double-wrapping yields a 0-byte
// payload host-side.

import { Schema, Field, Int64, Table, tableToIPC, vectorFromArray } from 'apache-arrow';

// Host imports — generated by jco from the WIT.
// Versioned import path matches the WIT `package platform:connector@0.1.0`.
// @ts-expect-error - resolved by jco at componentize time
import { log, httpFetch } from 'platform:connector/host@0.1.0';

function helloSchema(): Schema {
    return new Schema([new Field('id', new Int64(), false)]);
}

function ipcSchemaBytes(): Uint8Array {
    // Single-row placeholder so apache-arrow infers concrete types
    // (empty arrays collapse to Null). Worker reads only the schema.
    const id = vectorFromArray(BigInt64Array.from([0n]));
    const t = new Table({ id });
